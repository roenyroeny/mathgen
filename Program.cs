...
namespace mathgen
{
    ...
    public class Matrix
    {
        ...
        public string Ctors
        {
            get
            {
                ...
                for (int y = 0, i = 0; y < height; y++)
                {
                    ...
                    {
                        str += $"{type.type.Name} _{Util.Letters[y]}{Util.Letters[x]}";
                ...
                for (int y = 0; y < height; y++)
                {
                    ...
                    {
                        str += $"_{Util.Letters[y]}{Util.Letters[x]}";
                        str += (x != width - 1) ? ", " : "";
                    }
                    str += ")";
                    str += (y != height - 1) ? ", " : "";
        ...
        public string RowColFunctions()
        {
            ...
            for (int x = 0; x < width; x++)
            {
                str += $"c[i].{Util.Letters[x]}";
                str += (x != width - 1) ? ", " : "";

            }
            str += $")); }}\n";

            str += $"\t{type.Name} col(int i) const {{ return {type.Name}(";
            for (int x = 0; x < height; x++)
            {
                str += $"{Util.Letters[x]}.c[i]";
                str += (x != width - 1) ? ", " : "";
        ...
        public string Functions
        {
            get
            {
                ...
                str += $"\tstatic {type.Name} identity(int i) {{ return {type.Name}(";
                for (int y = 0, i = 0; y < height; y++)
                {
                    for (int x = 0; x < width; x++, i++)
                    {
